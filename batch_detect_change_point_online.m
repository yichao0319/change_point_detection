...
function batch_detect_change_point_online


    %% ----------------------
    % variables
    figure_dir = '/v/filer4b/v27q002/ut-wireless/yichao/mobile_streaming/change_point_detection/FIGURE_ONLINE/';
    output_dir = '/v/filer4b/v27q002/ut-wireless/yichao/mobile_streaming/change_point_detection/PRED_ERR_ONLINE/';
    ...
    for f_i = 1:length(input_files)
        ...
        for rank_i = 1:length(rank_methods)
            ...
            for filter_i = 1:length(filter_methods)
                ...
                %% ----------------------
                % Detect change points with given parameters, online version
                this_ts = ts;
                ts_len = length(this_ts);
                prev_cp = 1;

                cps = [];
                for ts_i = 10:update_intvl:ts_len
                    fprintf('.');
                    this_ts_sofar = this_ts(prev_cp:ts_i);
                    this_cps = detect_change_points(this_ts_sofar, num_bootstrap, conf_threshold, is_rank_based, is_filtered, 'no_plot') + prev_cp - 1;

                    cps = sort(unique([cps, this_cps]));
                    if length(cps) ~= 0
                        prev_cp = cps(1, end);
                    end
                end
